...
public final class JdbcJourneyStore implements JourneyStore {
    private static final String COLUMNS = "id, owner_id, kind, status, started_at, completed_at";
    private final JdbcTemplate jdbc;
    private final TransactionTemplate transaction;

    public JdbcJourneyStore(JdbcTemplate jdbc, PlatformTransactionManager manager) {
    ...
    public Journey start(UUID actorId, UUID journeyId, Journey.Kind kind, Instant now) {
        ...
        try {
            ...
            // PostgreSQL's partial unique index arbitrates simultaneous starts, across replicas.
            throw new JourneyConflict();
        } catch (org.springframework.dao.DataIntegrityViolationException invalid) {
            // The sole journey FK is owner -> account; deletion may win after authentication.
            for (Throwable cause = invalid; cause != null; cause = cause.getCause()) {
                if (cause instanceof SQLException sql && "23503".equals(sql.getSQLState()))
                    throw new SecurityException("Account is unavailable");
            }
            throw invalid;
        }
    }
