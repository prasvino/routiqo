...
public final class NativeAuthGuard extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        boolean binding = bindingEnabled
                && ("GET".equals(request.getMethod()) || "POST".equals(request.getMethod()))
        ...
        boolean spots = spotsEnabled
                ...
                || contributionsEnabled && "POST".equals(request.getMethod())
                    && ("/api/v1/native/spots/signals".equals(path) || "/api/v1/native/spots/posts".equals(path)
                        || path.matches("/api/v1/native/spots/items/[0-9a-f-]{36}/(?:vote|delete|reports|block-author)"));
        ...
        if (protectedOperation || journey || consent || routing || binding || spots) {
            ...
            if (journey || consent || routing || binding || spots)
                request.setAttribute(ACCOUNT_ATTRIBUTE, accountId);
        }
