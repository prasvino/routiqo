...
public final class AdminTrafficGrantController {
    ...
    private final JdbcTrafficGrantAdmin grants;
    ...
    @GetMapping("/me") Capability capability(HttpServletRequest request) {
        ...
        allow(actor, "admin-grant-read", 30);
        return new Capability(grants.canManage(actor, recheck(actor, request)));
    }
    @GetMapping("/{targetId}") JdbcTrafficGrantAdmin.Review review(@PathVariable String targetId,
    ...
    @PostMapping("/{targetId}/issue") JdbcTrafficGrantAdmin.Receipt issue(@PathVariable String targetId,
            HttpServletRequest request) { return change(targetId, "ISSUE", request); }
    @PostMapping("/{targetId}/revoke") JdbcTrafficGrantAdmin.Receipt revoke(@PathVariable String targetId,
            HttpServletRequest request) { return change(targetId, "REVOKE", request); }
    private JdbcTrafficGrantAdmin.Receipt change(String targetId, String action, HttpServletRequest request) {
        noQuery(request);
        UUID actor = actor(request);
        Input input = input(request, action);
        allow(actor, "admin-grant-write", 10);
    ...
    private void allow(UUID actor, String category, int limit) {
        try { if (!rates.allow(actor.toString(), category, limit)) throw new JdbcTrafficGrantAdmin.Limited(); }
        catch (JdbcTrafficGrantAdmin.Limited limited) { throw limited; }
    ...
    @ExceptionHandler(JdbcTrafficGrantAdmin.Denied.class) ResponseEntity<Void> denied() { return ResponseEntity.status(403).build(); }
    @ExceptionHandler(JdbcTrafficGrantAdmin.Missing.class) ResponseEntity<Void> missing() { return ResponseEntity.notFound().build(); }
    @ExceptionHandler(JdbcTrafficGrantAdmin.Conflict.class) ResponseEntity<Void> conflict() { return ResponseEntity.status(409).build(); }
    @ExceptionHandler(JdbcTrafficGrantAdmin.Limited.class) ResponseEntity<Void> limited() {
