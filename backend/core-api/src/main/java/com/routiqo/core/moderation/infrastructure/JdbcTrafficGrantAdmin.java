...
/** Exact-account V3 grants. Account locks precede grant locks in every operation. */
public final class JdbcTrafficGrantAdmin {
    private static final int AUDIT_CAPACITY = 1000;
    private final JdbcTemplate jdbc;
    private final TransactionTemplate transaction;
    public JdbcTrafficGrantAdmin(JdbcTemplate jdbc, PlatformTransactionManager manager) {
        this.jdbc = jdbc;
    ...
    public boolean canManage(UUID actor, Runnable sessionRecheck) {
        ...
        return transaction.execute(status -> {
            ...
            Window grant = lockGrant(actor, "traffic_grant_admin");
            sessionRecheck.run();
            Instant now = now();
    ...
    public Review review(UUID actor, UUID target, Runnable sessionRecheck) {
        ...
        return transaction.execute(status -> {
            ...
            Window administrator = lockGrant(actor, "traffic_grant_admin");
            Window targetAdministrator = lockGrant(target, "traffic_grant_admin");
            List<Object[]> locked = jdbc.query("""
                    SELECT permission, issued_at, expires_at FROM moderation_operator_grant
                    WHERE operator_id = ? AND permission IN ('traffic_review', 'traffic_suppress')
            ...
            noAdminTarget(targetAdministrator, now);
            ...
            readCapacity(actor);
            jdbc.update("""
                    INSERT INTO traffic_grant_read_audit_v3(id, administrator_id, occurred_at, expires_at)
                    VALUES (?, ?, ?, ?)
    ...
    public Receipt change(UUID actor, UUID target, UUID requestId, String permission, String action,
        ...
        if (requestId == null || requestId.equals(new UUID(0, 0))
                || !List.of("traffic_review", "traffic_suppress").contains(permission)
                || !List.of("ISSUE", "REVOKE").contains(action)
                || !List.of("OPERATOR_TRIAL", "COVERAGE_CHANGE", "SECURITY_RESPONSE", "ERROR_CORRECTION").contains(reason)
                || (action.equals("ISSUE") ? durationMinutes == null || durationMinutes < 15 || durationMinutes > 240
        ...
        return transaction.execute(status -> {
            ...
            List<Audit> audits = jdbc.query("""
                    ...
                    FROM traffic_grant_action_audit_v3
            ...
            if (!audits.isEmpty()) {
                ...
                if (!old.target().equals(target) || !old.permission().equals(permission)
                        || !old.action().equals(action) || !old.reason().equals(reason)
            ...
            if (action.equals("ISSUE")) {
                ...
                audit(actor, target, requestId, permission, action, reason, durationMinutes, expires, now);
            ...
            audit(actor, target, requestId, permission, action, reason, null, null, now);
    ...
    private static void noAdminTarget(Window grant, Instant now) {
        if (grant != null && grant.current(now)) throw new Missing();
    }

    private void writeCapacity(UUID actor, Instant now) {
        int retained = jdbc.query("SELECT request_id FROM traffic_grant_action_audit_v3 WHERE administrator_id = ? LIMIT 1001",
                (rs, n) -> rs.getObject(1, UUID.class), actor).size();
        int hourly = jdbc.query("""
                SELECT request_id FROM traffic_grant_action_audit_v3
    ...
    private void readCapacity(UUID actor) {
        if (jdbc.query("SELECT id FROM traffic_grant_read_audit_v3 WHERE administrator_id = ? LIMIT 3001",
