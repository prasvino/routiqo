...
/** Native boundary for the owner's completed trip journal. */
@RestController
...
public final class NativeJournalController {
    ...
    public NativeJournalController(JournalService journals, AuthRateGate rates) {
        this.journals = journals; this.rates = rates;
    }
    ...
    @PostMapping JournalResponse save(@PathVariable String id, HttpServletRequest request) {
        UUID actor = actor(request);
        if (!rates.allow(actor.toString(), "journal-write-account", 20)) throw new TooManyRequests();
        var input = NativeJournalJson.save(request);
        var mutation = new JournalMutation(input.title(), input.notes(), input.expectedVersion(), input.mutationId());
    ...
    @ExceptionHandler(JournalConflict.class) ResponseEntity<Void> conflict() { return ResponseEntity.status(409).build(); }
