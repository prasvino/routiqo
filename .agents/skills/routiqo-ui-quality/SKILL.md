...
Start with repository `AGENTS.md` and `docs/design/ROUTIQO_UI_UX_SYSTEM.md`.
Read the relevant feature spec and current tokens/components only as needed.
Treat the incumbent app as an existing product. Missing PRODUCT.md or DESIGN.md
does not authorize rebranding, replacing its fonts, or inventing product behavior.
...
- Preserve Home, Explore, Trips, Profile. Commutes and trips have different
  workflows; rich journals are not commute summaries. Conversation belongs to an
  active journey, not a permanent social tab.
...
- LIVE must not imply verified presence, safe roads, exact crowd counts or fresh
  data without evidence. Preserve stale/suppressed states, Ghost/consent controls,
  source attribution and privacy disclosures. UI polish cannot activate a closed
  publication gate or use synthetic signals as real observations.
...
then confirm the affected paths. Record inaccessible/device-dependent checks as
