...
2026-09-12 clarification: native account removal uses a durable account-ID-only retirement table, checked inside every queue mutation and acknowledgement transaction. Removing rows alone cannot prevent a delayed queue updater from recreating a deleted partition. The marker is permanent until app storage removal; it is not used for logout or cache reset. Initialization is additive, and retirement plus queue/snapshot removal is atomic. See `NATIVE_PARTITION_RETIREMENT_SPEC.md`.
