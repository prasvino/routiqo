# Planning storage failure recovery

## Objective and user value

Prove a blocked storage read or failed transformation cannot overwrite saved plans, and a subsequent explicit attempt can recover.
...
updateStoredPlans reads the current stored value through PlanningStorage, validates it, applies the transform, writes the result, and only then returns it. Existing tests cover latest-state merge, write failure and corrupt input; they do not exercise a denied read or a throwing transform.
...
**Only implementation edit:** `tests/planning-storage.test.ts` — extend the existing suite and reuse its fixtures.

Scheduling: Independent; owns only its test file.

## Functional requirements and implementation steps

1. Extend the existing injected in-memory PlanningStorage fake. Make getItem throw a synthetic SecurityError; spy on the transform and setItem.
2. Assert the original failure propagates, transformation/write never occur and the stored string is unchanged.
3. In a separate case let reading succeed but make the transform throw; assert no write and unchanged persisted plans.
4. Remove each synthetic failure, change the fixture's current saved state, and perform an explicit successful update. Assert the new attempt reads and transforms that current state, rather than stale cached state.

## Acceptance criteria and required tests

Both failure paths preserve saved data; the explicit recovery succeeds from freshly read state. Tests exercise updateStoredPlans itself, not a mocked replacement.
...
Use valid existing planning fixtures. Do not add a storage reset, automatic retry, fallback empty plan or backup format change.
...
```powershell
pnpm exec prettier --write tests/planning-storage.test.ts
pnpm exec prettier --check tests/planning-storage.test.ts
pnpm exec eslint tests/planning-storage.test.ts --max-warnings 0
pnpm exec vitest run tests/planning-storage.test.ts --maxWorkers=2
git diff --check
git status --short
```
