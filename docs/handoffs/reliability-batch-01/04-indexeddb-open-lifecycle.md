# Bound unavailable IndexedDB opens and dispose late handles

## Objective and user value

Ensure blocked storage cannot hang a user operation or leak a database handle when an open completes too late.
...
Both adapters' private open functions reject blocked/error requests and use a 5,000ms timeout. A late success closes the connection; a late upgrade aborts its transaction. Existing storage tests do not directly cover this open-request lifecycle.
...
- `apps/web/lib/journey-storage.ts`
- `apps/web/lib/journal-storage.ts`
- `tests/browser-journey-storage.test.ts`
- `tests/browser-journal-storage.test.ts`
- `docs/features/journey/WEB_JOURNEY_STORAGE_SPEC.md`
- `docs/features/journey/WEB_JOURNAL_STORAGE_SPEC.md`

**Only implementation edit:** `tests/browser-storage-open-lifecycle.test.ts` — proposed NEW test file; it does not yet exist.

Scheduling: Independent; owns a new test file.

## Functional requirements and implementation steps

1. Create the named new test file and parameterize public readBrowserJourneyPartition(account) and readBrowserJournal(account, journeyId). Do not export or mock the private open function.
2. Provide a narrowly typed controlled indexedDB.open request harness with event handlers and close/transaction spies. Trigger onblocked and separately advance fake timers to the existing 5,000ms timeout; attach rejection expectations before events.
3. Assert each operation rejects with its existing fixed unavailable-storage error. Deliver late onsuccess; assert close occurs and no data transaction begins. Exercise late onupgradeneeded and assert its transaction is aborted.
4. Restore globals and real timers before any real fake-indexeddb operation. Using a fresh IDBFactory, verify a subsequent explicit read succeeds with the adapter's empty result. Restore all spies/timers after each case.

## Acceptance criteria and required tests

Both adapters settle blocked/timed-out reads, close late connections, abort late upgrades and allow a fresh explicit read after availability returns; no real five-second waits or leaked test globals.
...
This is an open-event boundary test, not a claim about physical storage or transactional durability. Fake timers must not surround real fake-indexeddb work. No timeout relaxation or production storage changes.
...
```powershell
pnpm exec prettier --write tests/browser-storage-open-lifecycle.test.ts
pnpm exec prettier --check tests/browser-storage-open-lifecycle.test.ts
pnpm exec eslint tests/browser-storage-open-lifecycle.test.ts --max-warnings 0
pnpm exec vitest run tests/browser-storage-open-lifecycle.test.ts --maxWorkers=2
git diff --check
git status --short
```
