# Retain editor text after a stale local acknowledgement

## Objective and user value

Ensure a successful server response is not shown as fully reconciled when another tab's draft prevents local acknowledgement.
...
JournalEditorSession.save durably saves first, posts the saved mutation, then calls acknowledgeBrowserJournalDraft. A false acknowledgement produces a reopen warning and does not clear draft state. Existing UI tests cover POST failure and pre-POST local CAS failure, but not this post-success false branch.
...
Scheduling: Run before task 06; both own journal-editor.test.tsx.

## Functional requirements and implementation steps

1. Reuse open(), journal(), the current dialog fixture and mocked adapter boundaries. Enter synthetic title/notes; have the POST resolve to the matching version-1 journal and acknowledgement resolve false.
2. Click Save to account and await the warning: 'Saved to your account, but a newer device draft was found. Reopen the journal before sending again.'
3. Assert title/notes remain, no standalone 'Saved to your account.' success notice is shown, and the existing local-draft/unsent indication is retained.
4. Assert acknowledgement received the exact account, journey, sent mutation and returned journal; one POST occurred and no automatic second save, discard or retry followed.

## Acceptance criteria and required tests

The false acknowledgement warns truthfully, retains authored text and draft state, and does not claim reconciliation or retry automatically.
...
Do not require an immediate exact retry after false acknowledgement: a genuinely newer tab draft requires reopening and current local CAS must remain authoritative. This task ends at the warning/retention boundary.
