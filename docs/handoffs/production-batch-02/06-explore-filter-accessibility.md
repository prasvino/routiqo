# Expose destination categories as a named control group
...
People navigating by assistive technology get meaningful context for the destination filters.
...
Explore places aria-label='Destination categories' on a generic div. Its intended accessible group context is not exposed reliably, even though each category button already has aria-pressed.

## Expected behavior and functional requirements

Give the existing category wrapper role=group and preserve its name, button semantics and styling. Do not introduce tablist/listbox behavior or alter filtering.
...
- `apps/web/components/explore.test.tsx`
...
1. Add the semantic role to the existing filters wrapper; keep aria-pressed and the current ordinary buttons.
2. Add a jsdom component suite querying getByRole('group', {name:'Destination categories'}). Use real local catalog search, with child dialog/card dependencies isolated only as needed.
3. Verify All is initially pressed, selecting a category updates exactly one pressed state and the visible count/results, and selecting All restores the collection.
4. Combine category selection with a query producing no matches and use the existing Explore all action; assert query and category reset, group remains named and results return. Do not add a new search feature.

## Acceptance criteria

- Assistive technology can identify the named filter group and each selected/unselected toggle.
- Current query/category intersection, result status and empty-state recovery stay unchanged.
- No keyboard interception, new focus order, CSS or tab navigation semantics.
...
This is a functional accessibility improvement, not a redesign. Task 01 owns the route page; do not edit that file here. DOM roles alone do not prove screen-reader or visual compatibility.
...
Keyboard through filters, confirm Space/Enter activates ordinary buttons and inspect the browser accessibility tree for the named group.
...
```powershell
pnpm exec prettier --write apps/web/components/explore.tsx apps/web/components/explore.test.tsx
pnpm exec prettier --check apps/web/components/explore.tsx apps/web/components/explore.test.tsx
pnpm exec eslint apps/web/components/explore.tsx apps/web/components/explore.test.tsx --max-warnings 0
pnpm exec vitest run apps/web/components/explore.test.tsx --maxWorkers=2
