# Clean up failed planning downloads
...
Blocked downloads no longer retain private backup blobs or orphan DOM nodes.
...
download creates a Blob URL, appends an anchor, clicks it, removes it and schedules URL revocation. If click throws, catch displays an error but both removal and the only revocation path are skipped. Repeated failed attempts retain anchors and backup blobs.

## Expected behavior and functional requirements

Always remove any appended download anchor. Revoke its Blob URL promptly if the attempt fails; retain the existing delayed revocation after a successfully requested download so browser consumption is not interrupted. Failure leaves the dialog open with the text fallback available.

## Exact scope and file ownership

Edit existing production file:

- `apps/web/components/planning-backup.tsx`

Create the following test file (proposed new file at inspection time):

- `apps/web/components/planning-backup.test.tsx`
...
1. Restructure only download resource ownership using local anchor/URL references and try/finally or equivalent. Track successful initiation without treating it as proof the file was saved.
2. Ensure failures after URL creation clean up the URL and anchor; failure before URL creation must not attempt revocation of an undefined URL. Keep the successful 60-second revocation schedule.
3. Add jsdom tests using the real component and Modal, mocked provider boundary and URL/click spies: click throws after append, URL creation throws, successful click and later revocation.
4. Assert failure has an in-dialog alert, no success notice, no leftover anchor/URL and no storage mutation. Verify a subsequent successful attempt still works. Exercise View backup text after failure.

## Acceptance criteria

- Every allocated failed-attempt URL is revoked and appended anchor removed.
- Successful initiation still closes with the truthful download-requested notice and delayed revocation.
- No automatic export, upload or change to backup format/restore logic.
...
Do not immediately revoke a successfully dispatched download, or claim browser file-write success. Keep synthetic backup content out of logs. Avoid refactoring selectFile or restore.
...
From Profile test normal JSON download and copyable text fallback. Simulated download failure must retain a usable dialog.
...
```powershell
pnpm exec prettier --write apps/web/components/planning-backup.tsx apps/web/components/planning-backup.test.tsx
pnpm exec prettier --check apps/web/components/planning-backup.tsx apps/web/components/planning-backup.test.tsx
pnpm exec eslint apps/web/components/planning-backup.tsx apps/web/components/planning-backup.test.tsx --max-warnings 0
pnpm exec vitest run apps/web/components/planning-backup.test.tsx --maxWorkers=2
