...
The admin application and API use a separately configured Google OAuth audience, a separate challenge, session and cookie namespace, an exact admin origin, CSRF protection, a short session, and a default-off staging flag. A consumer session or caller-supplied operator UUID never grants access. The Google subject must resolve to an existing enabled Routiqo account; no admin sign-in creates accounts. Every read and write requires a current finite database permission. Admin identity-provider MFA policy and finite grant provisioning are external staging prerequisites; no role is inferred from email or domain.
...
4. OpenAPI/generated transport, configuration and operator runbook match the implementation. Real staging OAuth, MFA policy, grant provisioning, multi-account/device verification and retention review remain explicit prerequisites.
