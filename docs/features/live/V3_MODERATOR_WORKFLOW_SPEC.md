...
- Reporting protocol ([ADR 0064](../../adr/0064-v1-reporting-protocol.md)): groups with an `UNSAFE` report are listed first, then newest; counts are reporter-free group counts; when projection content is purged, a still-open group closes automatically as `CLOSED_EVIDENCE_UNAVAILABLE` (a system disposition with no operator, never replacing a suppression or a current dismissal). No count triggers automatic suppression.
